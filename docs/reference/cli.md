...
`--data-dir` stores a versioned atomic `catalog.json` snapshot containing complete canonical project archives and allocator metadata. The server restores it before binding its listener and rejects corrupt or unsupported snapshots. Project contents survive restart; command replay and idempotency history do not yet.
