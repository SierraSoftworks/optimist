...
Optimist samples the resolved equation and recommends an effective primitive family compatible with the requested support by matching sampled mean and variance. A stored estimate has exactly one active source: a directly authored distribution or a persisted Fermi equation. Fermi estimates retain their equation, variables, canonical formula, sampling controls, assessment diagnostics, interval, and effective distribution; existing analyses consume that effective distribution. Saving a direct distribution later replaces the Fermi source rather than layering two competing priors. Canonical project archives retain the complete embedded source.
