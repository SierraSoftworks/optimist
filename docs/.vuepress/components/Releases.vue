    ...
    <div class="release-list" v-if="selectedPlatform">
      <div class="release" v-for="release in applicableReleases" :key="release.id">
        <h4 class="release__name">
          <a class="release-button no-external-link-icon"
            :href="getReleaseAsset(release, selectedPlatform)?.browser_download_url" target="_blank">Download</a>
...
export default defineComponent({
  ...
  setup(props) {
    ...
    fetch(`https://api.github.com/repos/${props.repo}/releases`, {
      ...
      .then((found) => {
        found.sort((a, b) => b.published_at.localeCompare(a.published_at));
