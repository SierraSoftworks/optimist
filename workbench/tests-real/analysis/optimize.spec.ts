...
test('creates, selects, edits, and analyzes a scenario', async ({ page }) => {
  ...
  await expect(panel.getByText(/converged|maximum samples reached|insufficient valid samples/, { exact: true })).toBeVisible()
  await expect(panel.getByText('Plan success', { exact: true })).toBeVisible()
  await expect(panel.getByRole('figure', { name: 'Delivery reliability relative improvement over time' })).toBeVisible()
  await expect(panel.getByText('Impact vs baseline', { exact: true })).toBeVisible()
