...
Select a node and use **Details** to edit its title, Markdown description, and JSON metadata with project/node revision guards. Factors and outcomes also expose **Estimate** for current/desired normalized-state Point or Beta priors with provenance. Metric inspectors show each edge-scoped measurement history and can append timestamped readings with sources and optional known measurement error. Immutable observation correction, intervention costs, and scenario editing remain under development.
