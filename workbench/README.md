...
Use the header download button to save the selected project as `.optimist.json`. The upload button previews archive identity/counts before restore. Replacing an existing project requires typing its project ID. Imported archives start a new replay lineage at their archived revision because portable archives do not carry server event logs. Model commands return after their project-local WAL is durable; the header briefly reports background snapshot compaction and exposes degraded persistence errors. Each filesystem project directory retains metadata, complete state, committed changes, and idempotent results independently. Known monolithic server snapshots migrate forward at startup only after integrity validation succeeds.
