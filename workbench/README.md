...
Select a node and use **Details** to edit its title, Markdown description, and JSON metadata with project/node revision guards. Factors and outcomes expose **Estimate** for current/desired normalized-state Point or Beta priors with provenance, plus revision-checked qualitative evidence records. Metric inspectors show their native quantity contract and current distribution alongside each edge-scoped measurement history; they can replace direct estimates with support-compatible Point, Normal, LogNormal, Beta, or Scaled Beta distributions, append timestamped readings with optional known measurement error, and preserve immutable correction chains. Native metric Fermi adoption remains disabled until server assessment accepts owner-specific units and support. Intervention inspectors edit typed duration and success estimates plus independent named cost dimensions. Scenario editing remains under development.
