...
The Rust API validates the derived goal unit, runs deterministic Monte Carlo, reports convergence and rejected draws, and recommends a support-compatible primitive with a central 90% interval by matching sampled moments. The recommendation is applied only after explicit confirmation and only when the goal unit matches the estimate slot; otherwise the decomposition remains a useful standalone assessment. The complete equation and variable elicitation are appended to estimate provenance. Moment matching preserves mean and variance, not tails or multimodality.
