...
**Optimize** mode is a full-screen scenario workspace. Each candidate includes its recursive `requires` execution plan: prerequisite durations add, success probabilities compound, and successful prerequisite effects propagate before the candidate. Results show total duration, plan success, qualitative synergies/conflicts, and preference-oriented percentage improvement or regression against baseline. Positive and negative impacts use semantic green/red treatment in tables and baseline-relative percentage uncertainty trajectories. Budgets, costs, bundles, and numeric synergy magnitudes remain separate roadmap work.

**Impediments** mode is a full-screen intervention readiness board rather than a graph view. It ranks interventions by hard blockers, compounded success, and expected duration; shows prerequisite-first execution steps with compact duration/probability distribution strips; and surfaces factor requirements, synergies, and conflicts.
