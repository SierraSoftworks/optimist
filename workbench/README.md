...
After a short debounce, the workbench sends source, seed, retained sample count, and owner-derived target unit to Optimist's Rust Squiggle runtime. The backend lints and evaluates the wrapped unit annotation in a concurrency- and deadline-bounded blocking worker, then returns family, mean, variance, median, and a central 90% interval. Expensive calculations cannot occupy Tokio request workers or create unbounded detached work; busy and timed-out assessments return actionable API errors. Browser code does not synthesize an authoritative estimate result. Stale responses are discarded, and source outside the slot's empirical support keeps Save disabled.
